...
def buildings_id(id):
    ...
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    cur.execute("select * from buildings where building_id=?", (id,))
...
def buildings_id_assets(id):
    ...
    cur.execute("select floor_id from floors where building_id=?", (id,))
...
def floors_id(id):
    ...
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    cur.execute("select * from floors where floor_id=?", (id,))
...
#   def floors_id_model(id):
    ...
    #   cur.execute('select asset from floors where floor_id=?;', (id,))
    #   ans = cur.fetchone()[0]
...
def rooms_id(id):
    ...
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()
    cur.execute("select * from rooms where room_id=?", (id,))
